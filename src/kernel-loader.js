...
export class KernelLoader{
 ...
 async compile(source,spec){
  const worker=this.getWorker();if(!worker){const {compile}=await import('../vendor/cuda-webshader/compiler/compiler.js');const {ast,kernel,...a}=compile(source,{entry:spec.entry,workgroupSize:spec.workgroupSize});return a;}
