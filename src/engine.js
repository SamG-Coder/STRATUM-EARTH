...
export class Engine{
 ...
 async setGenome(value){const genes=validateGenomeFile(value);if(value.planHash){const bytes=new Uint8Array(await crypto.subtle.digest('SHA-256',this.planData));const hash=Array.from(bytes,b=>b.toString(16).padStart(2,'0')).join('');if(hash!==value.planHash)throw Error('This genome was farmed for a different shape plan.');}await this.runtime.idle();this.genome=value;this.genes=genes;this.input.fill(0);this.runtime.write(this.buffers.G,genes);this.runtime.batch().clear(this.buffers.World).clear(this.buffers.History).dispatch(this.bind('initCamera',{seed:genes[0],interiorSeed:genes[10]}),[1]).submit();await this.rebuild();}
