...
try{
 ...
 browser=await chromium.launch({headless:true,args:['--no-sandbox','--enable-unsafe-webgpu','--use-angle=swiftshader','--use-webgpu-adapter=swiftshader']});
 ...
 await page.waitForFunction(()=>!!window.stratumEarth,{},{timeout:45000});await page.waitForTimeout(1500);
 const report=await page.evaluate(async()=>{
  const e=stratumEarth.renderer,r=e.renderer,T=await import('three');r.setAnimationLoop(null);
  ...
  r.setRenderTarget(null);rt.dispose();await r.renderAsync(e.scene,e.camera);await r.backend.device?.queue.onSubmittedWorkDone();
  const c=e.canvas,s=getComputedStyle(c);return {colors:colors.size,sample:Array.from(pixels.slice(0,32)),info:r.info.render,canvas:{width:c.width,height:c.height,clientWidth:c.clientWidth,clientHeight:c.clientHeight,rect:c.getBoundingClientRect().toJSON(),display:s.display,opacity:s.opacity,zIndex:s.zIndex},camera:{position:e.camera.position.toArray(),projection:e.camera.projectionMatrix.toArray(),world:e.camera.matrixWorld.toArray()},globe:{visible:e.globe.visible,vertices:e.globe.geometry.attributes.position.count,matrix:e.globe.matrixWorld.toArray()},atCentre:document.elementsFromPoint(720,480).map(n=>n.tagName+'#'+n.id)};
 });
