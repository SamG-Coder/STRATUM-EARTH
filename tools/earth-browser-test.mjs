...
const base=`http://127.0.0.1:${server.address().port}${prefix}/`,software=process.env.CW_SOFTWARE_GPU!=='0';
let browser,page;const errors=[],tests=[],network=[],resourceErrors=[];
try{
 browser=await chromium.launch(browserOptions({software}));const context=await browser.newContext({viewport:{width:1440,height:960}});page=await context.newPage();
 ...
 assert.deepEqual(errors,[]);assert.equal(await page.evaluate(()=>!!stratumEarth.renderer.renderFailure),false,'Rendering device must remain healthy');
