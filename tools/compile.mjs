...
import {compile} from '../vendor/cuda-webshader/compiler/compiler.js';import {UNITS,SPECS} from '../src/kernel-specs.js';
...
for(const spec of SPECS){try{
 const source=spec.dependencies.map(n=>texts[n]).join('\n'),hash=createHash('sha256').update(stamp+'|'+spec.entry+'|'+spec.workgroupSize.join(',')+'|'+source).digest('hex');
 const artifact=compile(source,{entry:spec.entry,workgroupSize:spec.workgroupSize});if(artifact.metadata.bindings.length>8)throw Error('Storage-buffer budget exceeded');if(artifact.metadata.workgroupStorageBytes>16384)throw Error('Shared-memory budget exceeded');
...
await fs.writeFile(new URL('Stratum.cu',root),'// Generated combined CUDA. Shared exact geometry; finite shape-plan city.\n'+UNITS.map(n=>'\n// ==== '+n+' ====\n'+texts[n]).join('\n'));
