...
__global__ void probeGrammar(const float* Inputs,float* Output){
 ...
 Sink s=newSink(2,make_float3(0.0f,0.0f,0.0f),make_float3(0.0f,0.0f,1.0f),FAR);s.target=index;s=cityGroup(l,g,s);
 ...
 Sink hit=newSink(0,ro,rd,FAR);hit=cityGroup(l,g,hit);Output[11]=hit.t;Output[12]=(float)hit.fid;
